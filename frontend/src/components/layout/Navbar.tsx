...
export const Navbar: React.FC<NavbarProps> = ({ activeDataset, onSelectDatasetClick, backendHealth }) => {
  return (
    <header className="border-b border-emerald-900/80 bg-[#0a1510] sticky top-0 z-40 px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
      <div className="flex items-center space-x-3 min-w-0">
        ...
        <div className="flex items-center space-x-2 min-w-0 flex-wrap">
          ...
          <span className="text-[10px] font-mono font-medium px-2 py-0.5 rounded bg-emerald-950/70 text-emerald-300 border border-emerald-900 whitespace-nowrap">
            v1.0.0
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto">
        {activeDataset ? (
          <button
            ...
            className="flex items-center space-x-2 w-full sm:w-auto px-3 py-1.5 rounded-lg bg-emerald-950/70 hover:bg-emerald-900/50 border border-emerald-900 text-xs transition-colors text-emerald-100"
          >
            <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-400" />
            <span className="font-mono text-white truncate max-w-[180px]">{activeDataset.filename}</span>
            <span className="text-[10px] text-slate-400 whitespace-nowrap">
              ({activeDataset.rows_count} rows × {activeDataset.cols_count} cols)
            </span>
          </button>
        ) : (
          <button
            onClick={onSelectDatasetClick}
            className="flex items-center space-x-2 w-full sm:w-auto px-3 py-1.5 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 text-xs transition-colors"
          >
        ...
        <div className="hidden md:block h-5 w-[1px] bg-emerald-900" />

        <div className="flex items-center space-x-1.5 text-emerald-300 text-xs font-mono whitespace-nowrap">
