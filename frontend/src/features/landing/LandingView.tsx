    ...
    <div className="space-y-16 pb-12">
      ...
      <section className="text-center pt-8 space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-center space-x-4 pt-4">
          <button
            ...
            className="flex items-center space-x-2 px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-semibold shadow-lg shadow-indigo-500/25 transition-all transform hover:-translate-y-0.5"
          >
      ...
      <section className="glass-panel rounded-2xl p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 text-center pt-4">
          {[
            { step: '1', title: 'Upload CSV', icon: UploadCloud, color: 'text-indigo-400' },
            { step: '2', title: 'Data Cleaning', icon: Layers, color: 'text-blue-400' },
            { step: '3', title: 'EDA Dashboard', icon: BarChart2, color: 'text-cyan-400' },
            { step: '4', title: 'AI Insights', icon: Sparkles, color: 'text-emerald-400' },
            { step: '5', title: 'Select Target', icon: Database, color: 'text-purple-400' },
            { step: '6', title: 'Train ML Model', icon: BrainCircuit, color: 'text-pink-400' },
            { step: '7', title: 'Download Model', icon: Download, color: 'text-amber-400' },
          ].map((item, idx) => {
            const Icon = item.icon;
            return (
              <div key={idx} className="glass-card rounded-xl p-4 flex flex-col items-center space-y-2 relative">
                <div className={`w-10 h-10 rounded-lg bg-slate-800 flex items-center justify-center ${item.color}`}>
